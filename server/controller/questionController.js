...
// 	console.log("User ID:", userid); // Log the userid to ensure it is populated
...
//       SELECT questions.questionid, questions.title, questions.description, users.username FROM questions JOIN users ON users.userid = questions.userid ORDER BY questions.id DESC`);
...
// 		});
// 	} catch (error) {
// 				        console.error("Error in selectquestion:", error.message);
// 		return res
// 			.status(StatusCodes.INTERNAL_SERVER_ERROR)
// 			.json({ msg: "Something went wrong. Please try again later." });
// 	}
